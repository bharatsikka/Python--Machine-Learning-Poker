...
#'Straight Flush']
    ...
    dicti= {'Nothing':0,'Pair':0, 'Flush':0, 'Straight':0, 'Three of a Kind':0,'Straight Flush':0}
    for i in range(n):
        d = ThreeCardPokerDeck()
        d.shuffle()
        hand = d.deal_hand()
        if(hand.get_label() == 'Flush'):
            dicti['Flush'] += 1
            ...
            dicti['Three of a Kind'] += 1    
        elif(hand.get_label() == 'Straight Flush'):
            dicti['Straight Flush'] += 1    
        else:
            dicti['Nothing'] += 1
            
 
    for j in dicti.keys():
        dicti[j] = dicti[j]/n*100
...
#    pass
    ...
    while(is_playing(player_hand)):
        if player_hand._compare(dealer_hand) > 0:
            if(dealer_hand>min_dealer_hand):
                return (c,2)
            else:
                return (c,1)
        elif player_hand._compare(dealer_hand) < 0: return (c,-2)
        elif player_hand._compare(dealer_hand) == 0: return (c,0)
    else:
        return (c,-1)
